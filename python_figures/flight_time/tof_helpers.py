...
def draw_trajectory(mass):
    ...
    ax11.set_xlabel('Time / $\mu$s', fontsize = d.x_axis_font)
    ax11.set_ylabel('Position / cm', fontsize = d.y_axis_font)
    ax11.set_xlim(0,22)
    ax11.set_ylim(-2,120) 
    ax11.tick_params(direction='in', length=d.ticklength, width=2, colors='k',labelsize=d.labelsize,axis='both',pad=d.pad)
    ...
    ins_plt.set_ylabel('$\Delta$pos / cm', fontsize=d.y_axis_font-1)
    ins_plt.set_yticks([])
    ins_plt.tick_params(direction='in', length=d.ticklength, width=2, colors='k',labelsize=d.labelsize-1,axis='both',pad=d.pad)
    ins_plt.set_xlim(0,22)

    plt.savefig('../../svg_figures/trajectory.svg')
